...
var sett = Data.load_data(Data.save_path)
var highScore = 0
...
func _ready() -> void:
	if sett.night:
		$Hurtbox/CollisionShape2D.shape.radius = 16
		$Hurtbox/CollisionShape2D.shape.height = 40
		$Area2D/CollisionShape2D.shape.radius = 16
		$Area2D/CollisionShape2D.shape.height = 42
	else:
		$Hurtbox/CollisionShape2D.shape.radius = 8.75
		$Hurtbox/CollisionShape2D.shape.height = 38
		$Area2D/CollisionShape2D.shape.radius = 11
		$Area2D/CollisionShape2D.shape.height = 38
...
func damage(dmg) -> void:
	...
	if health<dmg:
		health = 0
		$HUD/Panel/HP.text = "Health: "+str(health)+"-"+str(dmg)
...
func heal(val) -> void:
	...
	if health<100:
		health += val
		$HUD/Panel/HP.text = "Health: "+str(health)+"+"+str(val)
		$HUD/Panel/HealthBar.value = health
	else:
		health = 100
		$HUD/Panel/HP.text = "Health: "+str(health)+"+"+str(val)
...
func scoreChange(val) -> void:
	score += val
	$HUD/Panel/Score.text = "Score: "+str(score)
	data.current = score
...
func _on_timer_3_timeout() -> void:
	...
	$HUD/Panel/HP.text = "Health: "+str(health)
