...
func spawn(s: bool) -> void:
	...
	if room_up2.name == "Room Cross":
		...
		$"../../Split 5/Room Up".add_child(roomChestV)
	...
	for i in $"../../../Rooms".get_children():
		for j in i.get_children():
			if j.name=="Room End" and j.get_children().is_empty():
				j.add_child(block)
			elif j.name=="Room Up" and j.get_children().is_empty():
				j.add_child(blockV)
			elif j.name=="Room End":
				j.remove_child(block)
			elif j.name=="Room Up":
				j.remove_child(blockV)
			else:
				pass
